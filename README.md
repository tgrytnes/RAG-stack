...
- `embedder` (Librarian, scaffolded): on a loop, ingests JSONs from `.staging` into Weaviate using Ollama embeddings (`EMBED_MODEL`, default `nomic-embed-text` at `http://host.docker.internal:11434`), then deletes the staging JSON; also watches `02_active` (`.md/.txt`) and re-embeds on change. Provides `--reindex` to reload all sidecars from archive. Requires the embedding model to be pulled in Ollama (e.g., `docker exec ollama ollama pull nomic-embed-text`). Exposes a search API on port 8001 -> POST `http://<pi-ip>:8001/search` with `{"query":"text","top_k":5}` returning Weaviate hits; health at `GET /health`.
 - `embedder` (Librarian, scaffolded): on a loop, ingests JSONs from `.staging` into Weaviate using Ollama embeddings (`EMBED_MODEL`, default `nomic-embed-text` at `http://host.docker.internal:11434`), then deletes the staging JSON; also watches `02_active` (`.md/.txt`) and re-embeds on change. Provides `--reindex` to reload all sidecars from archive. Requires the embedding model to be pulled in Ollama (e.g., `docker exec ollama ollama pull nomic-embed-text`). Exposes a search API on port 8001 -> POST `http://<pi-ip>:8001/search` with `{"query":"text","top_k":5}` returning Weaviate hits; health at `GET /health`. Also exposes an OpenAI-compatible Chat API at `http://<pi-ip>:8001/v1` with model id `weaviate-search` (use in Open WebUI as a custom OpenAI endpoint).
